...
function(check_command CMDNAME)
  ...
  string(COMPARE NOTEQUAL ${${CMDUPPER}} "${CMDUPPER}-NOTFOUND"
                 "HAVE_${CMDUPPER}")
...
function(compile_dummy)
  if(NOT EXISTS ${DUMMY_PROG_PATH})
    try_compile(
      DUMMY_COMPILE_SUCCESS SOURCE_FROM_CONTENT
      "TryCompileCheck.c" "int main(int ac, char* av[]) { return 0; }"
      COPY_FILE ${DUMMY_PROG_PATH})
    if(NOT DUMMY_COMPILE_SUCCESS)
      message(
        FATAL_ERROR "Cannot compile dummy program, check your environment!")
    endif()
  endif()
endfunction()
...
function(try_compile_check SHOWNAME VARIABLE SOURCE)
  if(NOT DEFINED ${VARIABLE})
    ...
    try_compile(COMPILE_SUCCESS SOURCE_FROM_VAR "TryCompileCheck.c" SOURCE)

    if(${COMPILE_SUCCESS})
      set(${VARIABLE}
          1
          CACHE INTERNAL "Have ${SHOWNAME}")
      if(NOT CMAKE_REQUIRED_QUIET)
...
function(executable_arch FILEPATH RESULT)
  ...
  # cmake does not support try-compiling an object file here we use DOS header
  # to distinguish PE files
...
function(add_dwz_of TARGET_NAME)
  ...
  set(STEM
      $<TARGET_FILE_PREFIX:${TARGET_NAME}_dwz>$<TARGET_FILE_BASE_NAME:${TARGET_NAME}_dwz>
  )
  ...
  add_custom_command(
    TARGET ${TARGET_NAME}_dwz
    POST_BUILD DEPENDS ${TARGET_NAME}
...
function(_add_debug_info_of TARGET_NAME SUFFIX FULL BUILD_ID)
  ...
  add_custom_command(
    TARGET ${TARGET_NAME}_${SUFFIX}
    POST_BUILD DEPENDS ${TARGET_NAME}
    COMMAND
      ${CMAKE_COMMAND} -D READELF=${READELF} -D OBJCOPY=${OBJCOPY} -D
      SRC=$<TARGET_FILE:${TARGET_NAME}> -D
      DST=$<TARGET_FILE:${TARGET_NAME}_${SUFFIX}> -D FULL=${FULL} -D
      BUILD_ID=${BUILD_ID} -P
      ${CMAKE_CURRENT_SOURCE_DIR}/extract-debuginfo.cmake
...
function(_add_minidebug_of TARGET_NAME SUFFIX V2)
  ...
  add_custom_command(
    ...
    POST_BUILD DEPENDS ${TARGET_NAME}
    COMMAND
      ${CMAKE_COMMAND} -D READELF=${READELF} -D OBJCOPY=${OBJCOPY} -D NM=${NM}
      -D XZ=${XZ} -D COMM=${COMM} -D SRC=$<TARGET_FILE:${TARGET_NAME}> -D
      DST=$<TARGET_FILE:${TARGET_NAME}_${SUFFIX}> -D V2=${V2} -P
      ${CMAKE_CURRENT_SOURCE_DIR}/extract-minidebug.cmake
    WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR})
endfunction()
